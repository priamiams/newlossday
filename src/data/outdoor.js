export const outdoorData = [
  {
    id: 1,
    title: 'Sewa Alat Pendakian Standar',
    location: 'Toko / Basecamp',
    image: '/img/dokumentasi/gambar5.jpg', // Using generic image
    description: 'Layanan sewa alat pendakian standar kami disediakan untuk memenuhi kebutuhan pendaki yang ingin melakukan perjalanan gunung dengan perlengkapan lengkap tanpa harus membeli perlengkapan pribadi. Seluruh peralatan dalam kondisi layak pakai, bersih, dan rutin dicek sebelum digunakan, sehingga aman dan nyaman untuk pendakian. Paket ini cocok untuk pendaki pemula hingga menengah, open trip, maupun private trip gunung.',
    features: [
      'Tenda gunung (kapasitas 2–4 orang, sharing)',
      'Carrier 50–70 liter',
      'Sleeping bag',
      'Matras',
      'Nesting (alat masak)',
      'Kompor portable',
      'Headlamp / senter',
      'Flysheet (jika dibutuhkan)'
    ],
    jalur: '-',
    duration: 'Harian / 2 Hari / 3 Hari / Custom',
    rundown: [
      'Pengambilan alat – Pengecekan kondisi – Penggunaan selama pendakian – Pengembalian alat'
    ],
    kuota: 'Menyesuaikan jumlah alat tersedia',
    jadwal: 'Setiap hari / By Request',
    price: 'Mulai Rp 150.000 / paket / hari',
    catatan: 'Kerusakan atau kehilangan alat menjadi tanggung jawab penyewa'
  },
  {
    id: 2,
    title: 'Sewa Tenda Gunung',
    location: 'Toko / Basecamp',
    image: '/img/dokumentasi/gambar5.jpg',
    description: 'Layanan sewa tenda gunung tersedia untuk pendaki yang membutuhkan perlindungan maksimal saat bermalam di alam terbuka. Tenda cocok digunakan di berbagai kondisi cuaca dan lokasi pendakian.',
    features: [
      'Tenda gunung waterproof',
      'Flysheet',
      'Pasak & tali tenda'
    ],
    jalur: '-',
    duration: 'Harian / Multi Day',
    rundown: [
      'Ambil tenda – Gunakan selama pendakian – Pengembalian'
    ],
    kuota: '2 – 4 orang / tenda',
    jadwal: 'Setiap hari',
    price: 'Mulai Rp 75.000 / hari',
    catatan: 'Kerusakan atau kehilangan alat menjadi tanggung jawab penyewa'
  },
  {
    id: 3,
    title: 'Sewa Carrier',
    location: 'Toko / Basecamp',
    image: '/img/dokumentasi/gambar5.jpg',
    description: 'Sewa carrier disediakan bagi pendaki yang membutuhkan tas gunung berkapasitas besar dan nyaman untuk membawa perlengkapan selama pendakian.',
    features: [
      'Carrier 50L / 60L / 70L',
      'Cover bag'
    ],
    jalur: '-',
    duration: 'Harian / Multi Day',
    rundown: [
      'Pengambilan carrier – Penggunaan – Pengembalian'
    ],
    kuota: '1 orang / carrier',
    jadwal: 'Setiap hari',
    price: 'Mulai Rp 40.000 / hari',
    catatan: 'Sesuaikan kapasitas carrier dengan postur tubuh. Kerusakan atau kehilangan alat menjadi tanggung jawab penyewa'
  },
  {
    id: 4,
    title: 'Sewa Sleeping Bag & Matras',
    location: 'Toko / Basecamp',
    image: '/img/dokumentasi/gambar5.jpg',
    description: 'Layanan sewa sleeping bag dan matras cocok untuk menjaga kenyamanan dan kehangatan saat bermalam di gunung. Perlengkapan ini sangat penting untuk menjaga suhu tubuh selama istirahat.',
    features: [
      'Sleeping bag',
      'Matras'
    ],
    jalur: '-',
    duration: 'Harian / Multi Day',
    rundown: [
      'Pengambilan alat – Penggunaan – Pengembalian'
    ],
    kuota: '1 orang / set',
    jadwal: 'Setiap hari',
    price: 'Mulai Rp 30.000 / hari',
    catatan: 'Disarankan menggunakan inner tambahan untuk kebersihan. Kerusakan atau kehilangan alat menjadi tanggung jawab penyewa'
  },
  {
    id: 5,
    title: 'Sewa Alat Masak Gunung',
    location: 'Toko / Basecamp',
    image: '/img/dokumentasi/gambar5.jpg',
    description: 'Sewa alat masak gunung disediakan untuk memenuhi kebutuhan logistik dan konsumsi selama pendakian. Peralatan praktis dan ringan, cocok digunakan di alam terbuka.',
    features: [
      'Nesting',
      'Kompor portable',
      'Tabung gas kecil (opsional)'
    ],
    jalur: 'Gunung tujuan pendakian',
    duration: 'Harian / Multi Day',
    rundown: [
      'Pengambilan alat – Penggunaan – Pengembalian'
    ],
    kuota: '1 set / kelompok',
    jadwal: 'Setiap hari',
    price: 'Mulai Rp 35.000 / hari',
    catatan: 'Gas bisa include atau exclude sesuai kesepakatan. Kerusakan atau kehilangan alat menjadi tanggung jawab penyewa'
  }
];
